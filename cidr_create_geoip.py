    ...
    with open(ipvfile, "r") as fd:
        for line in fd:
            ...
            print(
                format(
                    "%s\t%s\t%s\t%s"
                    % (attr.bin_addr(naddr), prefix, rec[0], regions[c])
                ),
                file=wfd,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-a", "--asn", default=False, action="store_true", required=False
    )
    parser.add_argument(
        "-c", "--country", default=False, action="store_true", required=False
    )
    parser.add_argument(
        "-C", "--city", default=False, action="store_true", required=False
    )
